      ...
      >
        <div className="h-full flex flex-col bg-white/6 backdrop-blur-sm border-l border-white/10 shadow-lg">
          ...
          <div className="px-4 py-3 flex items-center justify-between border-b border-white/10">
            <h3 className="text-lg font-medium text-gray-900">Cart <span className="text-sm text-gray-500">({localItems.length})</span></h3>
          ...
          <div className="p-4 flex-1 overflow-y-auto">
            {hasItems ? (
              <ul className="space-y-4">
                {localItems.map((it) => (
                  <li key={it.productId} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 rounded-lg bg-white/6 backdrop-blur-sm border border-white/6">
                    <div className="flex items-start sm:items-center gap-3 w-full sm:w-3/4">
                      {it.image ? (
                        ...
                        <div className="w-16 h-16 bg-white/8 rounded flex items-center justify-center text-sm">No Img</div>
                      )}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center rounded border bg-white/5 text-sm"
                          ...
                          <button
                            ...
                            className="ml-0 sm:ml-3 text-red-500 hover:text-red-600 flex items-center gap-1 text-sm"
                    ...
                    <div className="mt-3 sm:mt-0 w-full sm:w-auto text-center sm:text-right">
                      <div className="font-semibold text-sm text-gray-900">{formatPrice(it.price * (it.qty ?? 1))}</div>
                      <div className="text-xs text-gray-500 mt-1">{formatPrice(it.price)} each</div>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          <div className="px-4 py-3 border-t border-white/10">
            <div className="flex flex-col sm:flex-row items-center sm:items-end justify-between gap-3">
              <div className="text-center sm:text-left w-full sm:w-auto">
                ...
                <div className="text-lg font-semibold text-gray-900">{formatPrice(subtotal)}</div>
              </div>
              ...
              {hasItems ? (
                <Link
                  to="/checkout"
                  onClick={onClose}
                  className="w-full sm:w-auto text-center px-4 py-2 rounded-md bg-[#ea2eBe] hover:bg-pink-600 text-white text-sm transition"
                >
                  Checkout
                </Link>
              ) : (
                <button
                  type="button"
                  className="w-full sm:w-auto text-center px-4 py-2 rounded-md bg-gray-300 text-gray-700 text-sm cursor-not-allowed opacity-80"
                  aria-disabled="true"
                  disabled
                >
